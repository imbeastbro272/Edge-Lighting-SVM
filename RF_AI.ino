 ...
 * BASE MODEL: RANDOM FOREST (50 trees) - led_rf_model.h
 *
 * Mirror of DT.ino with the frozen base predictor swapped from a single
 * Decision Tree (tree_rules.h) to a Random Forest of 50 averaged trees
 * (led_rf_model.h::predict_rf). The KNN time-local incremental-learning
 * pipeline, EEPROM layout, sample format and serial-monitor interface are
 * intentionally identical to DT.ino.
 *
 * APPROACH:
 *   - The pre-trained Random Forest stays FROZEN as the base predictor
 *     (led_rf_model.h, predict_rf). It is never modified.
 ...
 *     average of (target - rf_at_sample) using ambient + motion as the
 *     distance features (time/day are already equal by virtue of the gate).
 *
 * PIPELINE per prediction:
 *   1. rf_pred  = predict_rf(features)
 *   2. eligible = filter(samples, same day AND |dt| <= window)
 *   3. if eligible empty -> return rf_pred (no correction at all)
 *   4. for each eligible sample:
 *        residual_i = target_i - rf_pred(sample_i)
 ...
 *   8. final = clamp(rf_pred + confidence * correction, 0, 100)
 *
 ...
 *   a stored sample back into the forest to compute its residual).
 *
 * COMMANDS:
...
// Bumped vs DT.ino so that flashing RF.ino over an existing DT.ino install
// safely wipes residuals/config that were tied to the Decision-Tree base.
#define KNN_CONFIG_MAGIC      0xC0FFEE03
#define SAMPLES_FORMAT_MAGIC  0xBEEF0003
...
float predict_knn(float ambient, int motion, int hour_of_day, int minute_in_hour,
                  int day,
                  float *out_rf_pred, float *out_correction,
...
static inline void sampleForestFeatures(const TrainingSample &s,
                                        float *sin_h, float *cos_h, int *period);

// ============================================
// RANDOM FOREST ADAPTER
// Single point of contact with the frozen base model. Mirrors the call
// signature used by DT.ino's predict_brightness(...) so the KNN logic
// below is structurally identical.
// ============================================

static inline float forest_predict(float ambient, int motion,
                                   float sin_h, float cos_h,
                                   int period, int day) {
  float f[N_FEATURES];
  f[FEAT_AMBIENT_LIGHT]   = ambient;
  f[FEAT_MOTION_DETECTED] = (float)motion;
  f[FEAT_SIN_HOUR]        = sin_h;
  f[FEAT_COS_HOUR]        = cos_h;
  f[FEAT_TIME_PERIOD]     = (float)period;
  f[FEAT_DAY_OF_WEEK]     = (float)day;
  return predict_rf(f);
}
...
void initializeTrainingStorage() {
  ...
  // previous firmware version with a different struct layout (or from DT.ino)
  // - wipe them.
...
void captureTrainingSample(float rf_pred, float user_brightness) {
...
// ============================================
// HELPER: derive forest-input features from a stored sample
// ============================================

static inline void sampleForestFeatures(const TrainingSample &s,
                                        float *sin_h, float *cos_h, int *period) {
  int hour = s.minute_of_day / 60;          // hour bucket for forest features
...
// ============================================
// KNN PREDICTION (Random Forest + Local Residual Correction)
//
...
float predict_knn(float ambient, int motion, int hour_of_day, int minute_in_hour,
  ...
  // 1. Frozen Random Forest (averaged 50 trees)
  float rf_pred = forest_predict(ambient, motion, sin_h, cos_h, period, day);

  if (out_rf_pred)        *out_rf_pred        = rf_pred;
  ...
  if (eligible_count == 0) {
    // No nearby samples for this exact time/day -> forest only.
    return rf_pred;
  }
  ...
  for (int k = 0; k < k_actual; k++) {
    ...
    sampleForestFeatures(s, &s_sin, &s_cos, &s_period);

    float rf_pred_at_s = forest_predict(
      s.ambient_light, s.motion_detected,
      s_sin, s_cos, s_period, s.day_of_week);

    float residual = s.target_brightness - rf_pred_at_s;
  ...
  float final_pred = rf_pred + confidence * correction;
...
void setup() {
  ...
  Serial.println(F("==================================="));
  Serial.println(F("LED Controller v4.1 - KNN (TIME-LOCAL) LEARNING (Random Forest)"));
  Serial.println(F("==================================="));
...
void loop() {
  ...
  if (!manual_input_mode && current_time - last_update >= UPDATE_INTERVAL) {
    ...
    float rf_pred, correction, confidence, min_dist;
...
void testManualPrediction(int hour, int minute, int day, float ambient,
  ...
  float knn_pred = predict_knn(ambient, motion, hour, minute, day,
                               &rf_pred, &correction, &confidence,
                               &min_dist, &neighbors_used, &eligible_count);

  float rf_final = rf_pred + offset;
  if (rf_final < 0) rf_final = 0; if (rf_final > 100) rf_final = 100;
  ...
  Serial.print(F("  RF raw            : ")); Serial.println(rf_pred, 2);
  ...
  Serial.println();
  Serial.print(F("  Final w/ pot : rf=")); Serial.print(rf_final, 2);
...
void cmdModelInfo() {
  ...
  Serial.print  (F("Base model     : Random Forest ("));
  Serial.print  (N_TREES); Serial.println(F(" trees, FROZEN)"));
  ...
  Serial.println(training_sample_count > 0 ? F("ACTIVE (RF + KNN)")
                                           : F("INACTIVE (RF only - no samples)"));
  Serial.println();
}
...
void handleSerialCommand() {
  ...
  if (command == "help") {
    ...
    Serial.println(F("    Forest-only mode is now active until new samples arrive."));
    Serial.println();
  }
